...
def inference(x, y, n_chars, embedding_size, filters, feature_size):
    ...
    with tf.name_scope('embedding'):
        w_emb = tf.get_variable("c_embed", [n_chars , embedding_size], trainable=False)
        ...
        x_emb = tf.transpose(tf.reshape(x_flatten_emb,[x.shape[0].value, 1, embedding_size, x.shape[1].value]), [0, 2, 3, 1])
        y_emb = tf.transpose(tf.reshape(y_flatten_emb,[y.shape[0].value, 1, embedding_size, y.shape[1].value]), [0, 2, 3, 1])
    ...
    with tf.name_scope('convLayers'):
        ...
        for i in range(len(filters)):
            W_conv[i] = weight_variable([embedding_size, filters[i], 1, feature_size], "conv_" + str(i) + "_weight")
    ...
    with tf.name_scope('maxpooling'):
        ...
        feature_x = tf.reshape(tf.concat(pooling_output_x, 1),[batch_size,len(filters)*feature_size])
        feature_y = tf.reshape(tf.concat(pooling_output_y, 1),[batch_size,len(filters)*feature_size])
        """if needed drop out here"""

    return feature_x, feature_y

def calloss(feature_x, feature_y, negtive_count, gamma):
    batch_size = feature_x.shape[0].value
    with tf.name_scope('FD_rotate'):
        # Rotate FD+ to produce 50 FD-
        temp = tf.tile(feature_y, [1, 1])
        x_feature = tf.tile(feature_x, [negtive_count + 1, 1])
        y_feature = tf.slice(feature_y, [0, 0], [0, -1])
        for i in range(batch_size):
            rotation = tf.concat([tf.slice(temp, [i, 0], [batch_size - i, -1]), tf.slice(temp, [0, 0], [i, -1])], 0)
...
def evaluation(cos_sim):
    ...
    cos_values = tf.reshape(v,[1,len(v)])
    ...
    v1 = tf.reduce_mean(tf.boolean_mask(cos_values, r1_mask))
    v3 = tf.reduce_mean(tf.boolean_mask(cos_values, r3_mask))
    v10 = tf.reduce_mean(tf.boolean_mask(cos_values, r10_mask))
    tf.summary.scalar('v1', v1)
    tf.summary.scalar('r3', v3)
    tf.summary.scalar('v10', v10)
